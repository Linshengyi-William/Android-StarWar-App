...
class PeopleQuizSuccess : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        Log.d("red", "onCreate() in PeopleQuizSuccess")
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_people_quiz_success)
        val image = this.findViewById<ImageView>(R.id.imageview_success)
        Picasso.get().load("https://pngimg.com/uploads/starwars/starwars_PNG1.png").resize(600,800).into(image)
        ...
        if(intent.hasExtra(USERNAME)){
            userCongrats.text = "Congratulations! ${username}!"
        }
